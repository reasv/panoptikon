...
| Note | restart and resume: the second job must start from the persisted anchor, not from the seed |
...
```bash
# results dir
DIR=$(python/.venv/bin/python tools/calibration-protocol/newrun.py --scenario S3 --run-id final-sm120)
