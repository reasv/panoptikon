...
def run_migrations_online():
    ...
    default_db_file, user_db_file, default_storage_db_file = get_db_paths()
    db_files = [default_storage_db_file]
    # Get other dbs
    index_dbs, user_data_dbs = get_db_lists()
    for index_db in index_dbs:
        index_db_file, user_db_file, storage_db_file = get_db_paths(
            index_db=index_db
        )
        if storage_db_file not in db_files:
            db_files.append(storage_db_file)

    for storage_db_file in db_files:
