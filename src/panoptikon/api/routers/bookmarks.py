...
@router.get("/", summary="Get all bookmark namespaces")
def get_ns_list(
    conn: sqlite3.Connection = Depends(get_db_readonly),
):
    return {"namespaces": get_all_bookmark_namespaces(conn)}
...
def get_bookmarks_by_namespace(
    ...
    return {"count": count, "results": files}
...
def delete_bookmarks_by_namespace(
    ...
    if items:
        ...
        return {"message": "Deleted bookmarks"}
    elif items is None:
        delete_bookmarks_exclude_last_n(
            conn,
            exclude_last_n,
...
def add_bookmarks_by_sha256(
    ...
    return {"message": "Added bookmarks"}


@router.delete(
    "/{namespace}/{sha256}", summary="Delete a bookmark by namespace and sha256"
)
...
def get_bookmark(
    ...
    return {"exists": exists, "metadata": metadata}
