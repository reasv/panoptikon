...
def create_whisper_ui(parent_tab: gr.TabItem):
    ...
    gr.Markdown(value=article)
    with gr.Row():
        with gr.Column():
            with gr.Tabs():
                with gr.Tab(label="Microphone"):
                    microphone = gr.Audio(sources=["microphone"], type="filepath")
                    microphone_button = gr.Button("Transcribe")

                with gr.Tab(label="Audio File"):
                    upload = gr.Audio(sources=["upload"], label="Audio file", type="filepath")
                    audio_button = gr.Button("Transcribe")

                with gr.Tab(label="YouTube"):
                    youtube = gr.Textbox(lines=1, placeholder="Paste the URL to a YouTube video here", label="YouTube URL")
                    youtube_button = gr.Button("Transcribe")
                    output_html = gr.HTML(label="Video")

            task = gr.Radio(["transcribe", "translate"], label="Task", value="transcribe")
            include_timestamps = gr.Checkbox(value=False, label="Return timestamps")

            with gr.Column():
                output_transcription = gr.Textbox(label="Transcription", show_copy_button=True)
                output_transcription_times = gr.Textbox(label="Transcription Time (s)")

    microphone_button.click(
        fn=transcribe_chunked_audio,
        inputs=[microphone, task, include_timestamps],
        outputs=[output_transcription, output_transcription_times],
    )

    audio_button.click(
        fn=transcribe_chunked_audio,
        inputs=[upload, task, include_timestamps],
        outputs=[output_transcription, output_transcription_times],
    )

    youtube_button.click(
        fn=transcribe_youtube,
        inputs=[youtube, task, include_timestamps],
        outputs=[output_html, output_transcription, output_transcription_times],
    )
