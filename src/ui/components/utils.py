...
from src.db import FileSearchResult, add_bookmark, remove_bookmark, delete_bookmarks_exclude_last_n, get_database_connection, get_all_bookmark_namespaces, get_bookmark_metadata, get_bookmarks

def toggle_bookmark(bookmarks_namespace: str, selected_files: List[FileSearchResult], button_name: str):
...
def on_selected_image_get_bookmark_state(bookmarks_namespace: str, selected_files: List[FileSearchResult]):
    if len(selected_files) == 0:
        return gr.update(value="Bookmark")
...
def get_thumbnail(file: FileSearchResult, big: bool = True):
    if file.type.startswith("video"):
        return f"./thumbs/{file.sha256}-grid.jpg" if big else f"./thumbs/{file.sha256}-0.jpg"
    else:
        return file.path