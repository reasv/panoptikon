...
function requestCard(item, state) {
  ...
  card.append(details);
  const rootsTitle = document.createElement('p'); rootsTitle.className = 'muted'; rootsTitle.textContent = 'Mapping these server folders to local folders is optional: it lets Relay open files and reveal folders in place. Copying files to the clipboard works even with no folders mapped. Edit them, match available folders on this computer, or leave them blank.'; card.append(rootsTitle);
