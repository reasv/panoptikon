...
//!
//! Port of `src/searchui/router.py` minus the git clone/pull: the checkout
...
/// 2. The repo venv's nodejs-wheel node, relative to `base` (the CWD). The
///    pip entry point (`.venv/Scripts/node.exe` / `.venv/bin/node`) is a
///    launcher stub that re-execs the real binary through Python, so the
///    actual node inside the `nodejs_wheel` package is preferred — running
///    it directly drops the Python hop and puts npm-cli.js findably next to
///    it. The stub remains a fallback.
/// 3. `node` from PATH.
...
fn venv_node_candidates(base: &Path) -> Vec<PathBuf> {
    ...
    if cfg!(windows) {
        vec![
            venv.join("Lib/site-packages/nodejs_wheel/node.exe"),
            venv.join("Scripts/node.exe"),
        ]
    } else {
