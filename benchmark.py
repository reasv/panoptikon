...
def select_random_tags(tags_list: List[Tuple[str, int]], min_tags: int = 1, max_tags: int = 3) -> List[str]:
    ...
    weights_num_tags = [2 ** -(i - min_tags) for i in range(min_tags, max_tags + 1)]
    
    # Normalize the weights to sum to 1
    total_weight = sum(weights_num_tags)
    weights_num_tags = [w / total_weight for w in weights_num_tags]
    
    num_tags = random.choices(range(min_tags, max_tags + 1), weights=weights_num_tags, k=1)[0]
    
    tags, counts = zip(*tags_list)
    
    return random.choices(tags, weights=counts, k=num_tags)

def filter_tags(tags: List[Tuple[str, int]], tags_to_remove: List[str]) -> List[Tuple[str, int]]:
    return [(name, count) for name, count in tags if name not in tags_to_remove]

def perform_search(conn, tags: List[Tuple[str, int]], page: int = 1, page_size: int = 10, folders: List[str] = [], use_total_count = True) -> Tuple[int, int]:
    ...
    res = list(search_files(
        ...
        setters=random.choices(V3_MODELS, k=random.randint(0, len(V3_MODELS))),
...
if __name__ == '__main__':
    ...
    tags = [(name, count) for namespace, name, count in get_most_common_tags(conn, limit=100)]
    print(len(tags))
    durations = []
    ...
    for i in range(1000):
        random.seed(i)
        start_time = time()
        returned, total = perform_search(conn, tags, page=1, page_size=page_size, folders=folders, use_total_count=use_total_count)
        ...
        if total > page_size:
            ...
            start_time = time()
            random.seed(i)
            total_last_page, _ = perform_search(conn, tags, page=total // page_size, page_size=page_size, folders=folders, use_total_count=use_total_count)
            print(f"Returned {total_last_page} files in {time() - start_time:.2f} seconds (page: {total // page_size})")
    ...
    print(f"Average results: {sum(n_results) / len(n_results):.2f} for total {len(n_results)} searches")
    print(f"Average duration: {sum(durations) / len(durations):.2f} seconds for total {len(durations)} searches")
    print(f"Average duration (no results): {sum(durations_no_results) / len(durations_no_results):.2f} seconds for total {len(durations_no_results)} searches")
    print(f"Average duration (with results): {sum(durations_with_results) / len(durations_with_results):.2f} seconds for total {len(durations_with_results)} searches")
    if len(duration_last_page_no_results) > 0:
        print(f"Average duration last page (no results): {sum(duration_last_page_no_results) / len(duration_last_page_no_results):.2f} seconds for total {len(duration_last_page_no_results)} searches")