...
| **Deslopping** | 2026-09-04 → 2026-09-05 | The user reviewed the PR size (+86 051 lines, ~19 000 of them product code) and asked for a comment/test/doc concision pass and an assess-first approach to code bloat first. Phase 1 landed (whole-PR insertions 86 255 → 72 594, production tokens byte-identical); **phase 2 landed on 2026-09-05** — 26 of the 27 approved rows in 36 commits, 627 insertions / 870 deletions over 22 files, verified behaviour-neutral with byte-identical `analyze.py` verdict JSON on two recorded legs | `docs/batch-calibration-deslop-plan.md` |
| **Phase 3 legs measured** | 2026-09-05 05:13–10:18 | The user authorised stopping SGLang; all seven remaining legs ran on the deslopped tip binary `63a8ad73` (image `5b2b3ed166c0` for S11-C4) and SGLang was restarted at the end. **S4g, S4d, S8 pixmix, S11-C4 and the 4 h soak PASS; S4b PASSes but its "within a few seconds" latency is bounded by the in-flight window; the shape ceiling is unreachable through a job on a 98 GB GPU.** No product defect in any leg; two tool fixes (`555213ac`, `176f6ad9`). Three items went to the user (report §6): S4b's expectation, the ceiling's reachability, and the soak's `oracle_agreement` (14.12 % → 13.53 % only) — the third has since been **answered by the row below**, as a product defect | run2 report §4.5, §4.7, §4.8 (per-leg expectation tables) and §6 |
| **`external_mb` fix measured** | 2026-09-05 12:24–13:28 | Characterising the soak's 8 221 `oracle_agreement` breaches offline named a **product defect**, not a tool artefact: `Ledger` booked an in-flight replica's own allocator pool as another process's memory (90.9 % of breaches; **0** whenever no grant was outstanding). The like-for-like run1 comparison is 3.06 %, not 14.12 %, so run2 was a 4.4× regression that scaled with grant size. Fixed by per-batch `memory` frames behind a `batch_memory_frames` handshake field, independently verified (six code questions plus a counterfactual), and measured on binary `0b6f0c66` against each leg's own comparand with SGLang stopped and restarted: **S6-contend-fix** — 1.3–1.9× throughput at 0 failures; **S9-fix** — breaches 15.78 % → **2.41 %**, invariant 6.87 % → **2.26 %**, OOM negatives 2 → **0**, grant sizes and wall times unchanged. Two new user items: fit `allocated` rather than `reserved`, and whether to re-record two legs' stored `verdicts.json` | run2 report §4.9 and §6 |
| **Representative model sweep** | 2026-09-05 21:19–22:01 | The user authorised stopping SGLang again and approved a representative sample: **34 of the 119 shipped inference ids** probed against the ledger's `base + slope × units` model on the **shipped** registry (one probe per GPU, `--empty-cache-between-sizes`, 55.1 min of probe wall inside a 40 min 27 s window; one tool commit, `bfbb31d8`, for the `--mode audio-npy` payload the whisper and clap impls read). **26 of the 34 are linear** and the 8 that are not are exactly three groups: `doctr/db_resnet50_*` bends identically in all three members (docTR's internal sub-batching), the shipped `easyocr_*` are **flat** under `enable_batching = false`, and `whisper` is unpriced CTranslate2. **`peak_allocated_mb` reproduced run1 on 39 of 39 shared points to within 3 MiB while `delta_mb` reproduced on none**, its error a per-model factor (0.525…1.043), and `reserved/allocated` runs 1.011…1.695, median 1.215, moving with the batch — so there is no global fragmentation factor. Four product defects recorded and not fixed (coca's fp16 default, clap's 16 kHz/48 kHz mismatch, `cudnn.py`'s in-process `LD_LIBRARY_PATH` — run1's F5 — and the 32-bit index ceiling as a general limit), plus three registry findings and one new user decision (deprecate the `moondream` ids) | run2 report §4.10, §6, and §2 for the artefacts |
...
- **B4 confirmed on all three legs, and the anchor is worse than a floor.**
  ...
  **4 096** (findings N5/B4c). The OOM backstop is the only thing under it.
  Since re-adjudicated: the anchor travels on any matching profile that also
  carries a slope, always as a seeded claim, and an out-of-memory window now
  halves one no clean batch on the reading card has reached — by its own error
  frame, by a batch's, or by killing the worker. An anchor this card has run
  still stands, as B4c recorded.
...
| Windows desktop, dual RTX 5090 (WDDM) | **No per-process oracle exists here** (measured 2026-09-06, driver 610.74): NVML returns N/A for every process and `nvidia-smi --query-compute-apps` returns `[N/A]` too, `-i <uuid>` does not scope its list, and `pmon` has no memory column. The oracle is therefore GPU-level used/free from NVML, plus our own worker's footprint from the worker's `/health` figures; `oracle_agreement`, `base_accuracy` and `footprint_agreement` SKIP. Expect base tier `free_delta` or `alloc_delta`. | Same (torch) | Sysmem fallback: over-admission shows as throughput collapse, not OOM; S4c must watch `throughput_collapse` flags and per-batch `duration_ms`; S15 mutation 1 is the key sensitivity test there. Also run once with the driver's "Prefer No Sysmem Fallback". Two GPUs: S7 is the monitor-asymmetry test the design defers. |
| MacBook Pro M3 Max 128 GB (MPS) | No per-process GPU counter; oracle records `vm_stat`/psutil RAM, `sysctl iogpu.wired_limit_mb`, and the worker's own `driver_allocated` from `/health`; ground truth for base comes from `ceiling_probe.py` in-process (`torch.mps.driver_allocated_memory()` is per-process by construction). **The oracle's RAM row must be the same formula the product uses** — `hw.memsize − wired − compressor − anonymous pageable` — not psutil's `available` and not free + inactive, both of which the 2026-09-06 pass disqualified (F1, F4). | RAM hog (numpy) and an MPS hog (torch tensors on `mps`); on macOS the hog must **re-touch** its pages every tick, or ageing hands them back to every reader within ~90 s, and `leave-free` prices with the same counter formula — priced with psutil's `available` a `leave-free 20480` leg left ~11 400 MiB (phase 2 defect 3) | Total adoption from the first load (`GPU-MPS`, recommended-max), re-adoption after raising the wired limit under a running gateway, watermark env 1.0/1.0, jetsam death-as-negative (S12 analogue). Settled by the 2026-09-06 pass, and now regression-tested rather than open: the free reading under a held hog (F1), the worker's in-process reading (F4), the out-of-memory comparand being the allocator's ceiling and not RAM (F3), and the near-ceiling GC bias, which is **−18 %** and is why the peak is sampled at 20 ms during the batch (F7). The field-pass list in `unified-memory-admission.md` maps onto S1/S3/S4/S12. |
...
|---|---|
| **`base_method` at load** (`grep 'base_method'`, or the `NVML lists no process with pid` warning) | Driver- and container-runtime-dependent, not container-shape-dependent. B9's predicted degradation did not happen here, so the degraded tiers — and W4's fixed 500 MiB context estimate against a measured 666–668 MiB context — are **untested everywhere so far**. Which of `free_delta` and `alloc_delta` answers depends on the **board's state**, not on the platform — the Windows 5090 gave one idle and the other under a hog, 1.9 % apart — so record the method per leg, not per host |
