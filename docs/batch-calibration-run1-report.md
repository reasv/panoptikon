...
| Check | Why |
|---|---|
| **`base_method` at load** (`grep 'base_method'`, or the `NVML lists no process with pid` warning) | Driver- and container-runtime dependent, not container-shape dependent: the degraded tiers and W4's fixed 500 MiB context estimate against a measured 666–668 MiB context were **untested everywhere** until the 2026-09-06 Windows pass, which found that which degraded tier answers depends on the board's state rather than the platform |
