...
| **The platform passes** | **Ampere and Windows done; MPS and ROCm not.** 18 legs on an RTX 3090 (16 PASS, 2 FAIL, 1 inconclusive) and 12 on a Windows/WDDM host with 2 × RTX 5090 (12 PASS, 0 FAIL), both on `5c778e48`. The Windows pass found and closed one ledger defect — the flat 4 096 MiB load reservation, `ba6708e4` + `911cd370` — and settled four platform facts run1 and run2 could only reason about: no per-process oracle at all, `base_method` following the board's state, the torch free tier over-reporting by 5.2 GB, and over-admission presenting as a throughput collapse. Run2 report §4.13, §4.14, §6, §10 |
| **The two Ampere FAILs** | **Both fixed and measured.** D2 (`560139ae`, merge `7d604052`): the worker's shrink rule reset on a zero-MB grant and the host's trim could only ask a neighbour, so a memory-blind window with ≥ 256 MiB of pool slack now counts as an under-grant window and a sole tenant can be asked to trim itself — on the S4b shape (`results/run2/D2-poolpin-{before,after}/`) memory-blind grants fall **956 → 2**, footprint **24 504 → 1 102 MiB**, headroom **0 → 13 025 within 33 s**, both legs 8 000/8 000 at +5.7 % inference time. D6 (merge `3180b383`, then `e7d276ae`, `fdb7cdba`, `ab0fa85a`, `744652a5`, `75efcffc`): the price, not the measurement — `bytes // 4` uncapped against a 256-token window; `metadata.cost.max_tokens` caps both sides and rides the grant, epochs bumped, and MiniLM re-measures **0.264× → 1.17×** (`results/run2/S2-minilm-tokenfit/`) with the wd-vit control unmoved at 1.0. The verifier's own finding — the worker priced text in characters where the host counted bytes, CJK off **2.64×** — is closed by `e7d276ae`. Run2 report §4.13, §6 |
| **Architecture-keyed profiles** | **Done and verified.** `daec9bfe` (merge `4bbba2b8`) keys the store on the GPU architecture with the SKU name kept as provenance, schema 2 → 3; three defect fixes followed (`7ef58fb9`, `df614c7c`, `98e8e0f0`). The cross-SKU leg proves it: the RTX 5090's stored slope priced the RTX PRO 6000's first window, and measured cold the same card fits **29.858823529411765**, bit-identical. **Open decision**: a profile in the local store confers its anchor, one in the baseline directory only its fit — documented, but whether anchor adoption should also be gated on the card is the user's call. Run2 report §4.15 |
