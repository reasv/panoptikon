...
| **The two Ampere FAILs** | **Fixes in progress, numbers owed.** D2 — one 512-unit window pins a 24 GB card, so 2 613 of 2 615 grants price blind and an external process is starved — is on `fix/pool-pin`. D6 — the learned token slope is 0.264× the probe's, and 0.004475 on this host under the allocated basis, so it is the fit and not the card — is on `fix/token-fit`. Neither is reachable on a 97 GB card |
