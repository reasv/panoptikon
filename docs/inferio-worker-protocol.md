...
| `trim` | — | Release the caching allocator's **unused pool** (`torch.cuda.empty_cache()`, or `torch.mps.empty_cache()` on Apple Silicon), keeping weights, live tensors and the CUDA context. Valid in every state; a worker with no live CUDA does nothing and still replies `ok`. Reply carries a fresh memory sample (below) and, **new**, `released_mb` / `release_ms`: MiB the release actually handed back and the `empty_cache()` call's own wall time. Both **absent unless the release ran and could be measured**, which is what tells a release that gave the card memory from one that gave it nothing — the reply is `ok` either way, so a reply is not evidence of a release. Never an error path: a trim that could not measure anything is still an `ok`. |
...
is believed between 16 and 1 000 000 tokens: below the floor is a
misidentified attribute (too *small* a cap under-prices an item, which
over-admits), and above the ceiling is a tokenizer's `int(1e30)` "no limit"
sentinel. Being a **count**, it is read
...
batch. That last property is why **neither peak decides whether the pool
grew**: an in-batch maximum is above the post-batch reading by construction, so
compared against `reserved_before_mb` it marks every batch pool-growing.
`reserved_after_mb` is the reading that answers it, on every backend. The caching pool is *usually* monotone between `empty_cache()`
