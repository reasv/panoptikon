...
- Accelerator env (`panoptikon/src/accelerator_env.rs`): when
  `[inference_local.python_env].accelerator` is `rocm` or `auto`, workers
  get host HIP/HSA on `LD_LIBRARY_PATH` (not for explicit `cpu`/`cuda`);
  setup `--accelerator rocm` probes a HIP kernel after sync (fails on wrong
  wheel arch / missing userspace).
