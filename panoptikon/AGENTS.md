...
- ROCm (`panoptikon/src/rocm_env.rs`): workers get host HIP/HSA on
  `LD_LIBRARY_PATH`; setup `--accelerator rocm` probes a HIP kernel after
  sync (fails on wrong wheel arch / missing userspace).
