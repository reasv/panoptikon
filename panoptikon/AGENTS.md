...
- Inferio orchestrator (`panoptikon/src/inferio/`), the Rust port of the Python inference server: `registry.rs` parses the inference TOML registry into per-id spawn specs; `worker.rs` supervises `python -m inferio_worker` child processes speaking the framed-msgpack protocol (`docs/inferio-worker-protocol.md` v2) — handshake (worker *identity* only: `protocol_version=2` + `impl_class` + `impl_dirs`, no instantiation; a version echo != 2 is a fatal kill), optional `prewarm` (runs the impl's optional `prepare()` classmethod between handshake and configure; idempotent, errors per-request and non-fatal; uses the LOAD deadline since prepare exists to pay the slow imports early), `configure` (binds a concrete model: instantiates `impl_class(**config)`, exactly once, before load; errors are per-request and do NOT poison the worker), then load/predict/trim/ping/unload (unload valid in every state — a parked prewarmed worker exits 0 the same way; `trim` is the orchestrator-initiated `empty_cache()` for an idle resident whose retained allocator pool is squeezing a neighbour — valid in every state, never an error path on the worker side, fixed 60 s deadline **not** floored by the configured handshake deadline — freeing a multi-GB pool is a `cudaFree` per block, not the milliseconds an idle `empty_cache()` costs, while the handshake deadline is about spawn liveness — and it replies with a fresh memory sample so the ledger stops charging the released slack; a trim that dies fatally logs a `warn!` naming itself before the teardown, since nobody asked for it and the model's death would otherwise have no visible cause). `Worker::spawn` does handshake only; `Worker::spawn_configured` chains spawn+configure for the normal flow (what `manager.rs::spawn_model` uses). Lifecycle deadlines per the protocol doc (handshake deadline covers configure/ping; prewarm gets the load deadline), single outstanding request enforced via `&mut self`, stderr forwarded to tracing with a bounded tail attached to error reports, per-request `error` frames surfaced as downcastable `WorkerError` (worker survives), framing violations/timeouts/exits treated as fatal (worker killed + poisoned), and graceful stop via the unload → terminate → kill ladder. Workers sit under `kill_on_drop` plus the shared kill-on-close Job Object (`panoptikon/src/process_tree.rs`, extracted from `jobs/files.rs` and also used by the HTML-thumbnail browser path).
  ...
  - `dispatch.rs` implements dispatch-time batching (design §6) over a multi-replica WorkerSet (design §8, Phase 3): per model, a plain tokio task + mpsc queue owns N worker replicas serving ONE shared FIFO queue — free replicas sit in a pool, in-flight windows run as `JoinSet` tasks that return their replica to the pool, and whenever any replica is free the queue is drained into a window for it, merged FIFO against per-window `WindowBounds` (units, items, payload bytes) computed *per replica*, since replicas can sit on different boards with different headroom. On the **priced** path the unit bound is the ledger's `window_target_units` (admitted units × `WINDOW_DEPTH_MULTIPLIER`) there is normally no item bound at all — the worker's packing harness splits the window into GPU batches inside the grant, so the dispatcher does not size batches; the ledger's `Grant` (unit budget, MB, cost unit/aggregation, `user_cap_items`) rides the request frame. A request's explicit `max_batch` does add one, `cap × WINDOW_DEPTH_MULTIPLIER`, so a cap of 1 cannot turn a window sized for thousands of units into thousands of one-item batches (which would overflow the telemetry ring and run for minutes before the grant is re-evaluated). On the **unpriced** path (no GPU inventory, `none`-class cost, a ledger that refused the grant) there is no worker-side packer, so the dispatcher bounds the frame itself by item count: the request's explicit `max_batch`, else the model's registry `default_batch_size` (group overlaid by id), else the server default (`ManagerConfig::default_max_batch`, replaces `MAX_COMBINED_BATCH`). Either way a **user cap partitions rather than shrinks**: capped and cap-less requests are not merged into one window, so a cap-1 request never drags a whole window down to one item. Request *pickup* is strictly FIFO (windows are queue prefixes); completion order across replicas may differ (per-request oneshot replies). Oversized single requests are split into sequential sub-batches; a merged batch failing with a `WorkerError` falls back to per-request prediction on the same replica (port of `process_model.py::_batch_predict`).
